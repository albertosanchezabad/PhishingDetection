...
DIRECTORIO_RESULTADOS = Path("resultados/lstm_models_kt")
DIRECTORIO_TUNER = Path("trials/lstm")
...
def construir_hipermodelo(hp, vocab_size, max_len):
    """Construye modelo con hiperparámetros optimizables (para Keras Tuner)"""
    model = Sequential()
    ...
    # Optimización de hiperparámetros de compilación
    optimizer_name = hp.Choice('optimizer', ['adam', 'rmsprop'])
    learning_rate = hp.Float('lr', min_value=1e-4, max_value=1e-2, sampling='log')
    
    # Crear instancia del optimizador correspondiente
...
def tuner_hyperparameter(X_train, y_train, X_test, y_test, vocab_size, max_len):
    ...
    tuner = kt.Hyperband(
        ...
        overwrite=False  # Cambiar a True para reentrenar desde cero
    )
...
def construir_modelo_base(vocab_size, max_len):
    """Construye modelo BiLSTM con parámetros base (sin optimizar)"""
    model = Sequential()
    model.add(Embedding(
        input_dim=vocab_size + 1,
